...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors().and().csrf().disable()
                .authorizeRequests()
                .antMatchers("/customer/addCustomer","/customer/authenticate", "/swagger-ui.html",
                        "/v2/api-docs","/webjars/**", "/customer/swagger-ui/", "/customer/api/swagger-ui/",
                        "/customer/get-customer-role/{role}").permitAll()
                .antMatchers("customer/get-customer/{username}","customer/customer-id/{id}",
                        "customer/get-all-customer","customer/update-profile/{username}"
                        ,"/customer/current-user","/customer/car-details","/customer/allpacks"
                        ,"/admin/customers","/customer/my-orders")
